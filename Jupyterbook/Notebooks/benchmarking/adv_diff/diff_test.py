...
mesh = uw.meshing.StructuredQuadBox(
    elementRes=(int(res), int(res)), minCoords=(xmin, ymin), maxCoords=(xmax, ymax), qdegree=2
)
...
with mesh.access(T):
    ...
    for r in range(len(T.data)):
        for c in range(len(T.data[r, :])):
            if (r > 10 and r < 20):
                 T.data[r][c] = tmax 
"""

with mesh.access(T):
    for index, coord in enumerate(T.coords):
        ##print(coord[1], minY + pipePosition, maxY-pipePosition, coord[1] >= minY + pipePosition and coord[1] <= maxY-pipePosition)
        print(coord[1], coord[1] <= 0.75 and coord[1] >= 0.25)
        if ( coord[1] <= 0.75 and coord[1] >= 0.25):
            T.data[index] = 1
        else:
            T.data[index] = 0
    print(T.data)
...
def plot_fig():
    if uw.mpi.size == 1:
        ...
        with mesh.access():
            ...
            pvmesh.point_data["T"] = uw.function.evaluate(T.sym[0], mesh.data)
...
### y coords to sample
sample_y = np.arange(
    T.coords[:, 1].min(), T.coords[:, 1].max(), T.get_min_radius()
)  ### Vertical profile
...
plt.savefig('Diffusion_benchmark_evolution.pdf', bbox_inches='tight', dpi=500)
# -
