...
## define the swarm
swarm = uw.swarm.Swarm(mesh=mesh, recycle_rate=0)
...
def saveState(mesh, swarm, v, p, v_star, differences, index, path=outdir):
    ...
    swarm.save_checkpoint(swarmName = 'swarm', swarmVars = [v_star], index = 0, outputPath=outdir)

    mesh.write_timestep_xdmf(filename = path, meshVars=[v, p], index=index)
...
def loadState(path=outdir):
    ...
    try:
        ...
        print("checkpoint3")
        swarm.load(path+'swarm-0000.h5')
        print("checkpoint4")
        v_star.load(filename=path+'Vs-0000.h5', swarmFilename=path+"swarm-0000.h5")
        print("checkpoint5")
